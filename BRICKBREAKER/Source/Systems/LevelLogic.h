...
namespace BBG
{
	...
	public:
		...
		bool Init(std::shared_ptr<flecs::world> _game,
			...
			GW::AUDIO::GAudio _audioEngine, std::vector<GamePacket> strPkt);
	...
	class ResetLogic
	{
		...
		flecs::system resetSystem;
		std::vector<GamePacket>* gmpt;
		std::vector<GamePacket> stpt;
		std::vector<int> idRollCall;
		int lvl;
		bool present;
	public:
		// attach the required logic to the ECS 
		bool Init(std::shared_ptr<flecs::world> _game, std::weak_ptr<const GameConfig> _gameConfig, std::vector<GamePacket>* gamepkt, std::vector<GamePacket> strpkt,
			GW::INPUT::GInput _immediateInput,
			GW::INPUT::GBufferedInput _bufferedInput,
			GW::INPUT::GController _controllerInput, bool& reset);
		// control if the system is actively running
		bool Activate(bool runSystem);
		// release any resources allocated by the system
		bool Shutdown();

...
public:
	...
	struct NameData { std::string name; GW::MATH::GVECTORF row4; NameData(std::string nm, GW::MATH::GVECTORF r4) { name = nm; row4 = r4; } };
	//0.5920, -0.0606,  0.8037, 0.0000
	...
	// All transform data used by each model
	std::vector<GW::MATH::GMATRIXF> cameraTransforms;
	...
	bool ReadGameLevel(const char* gameLevelPath, 
		...
		while (+file.ReadLine(linebuffer, 1024, '\n'))
		{
			...
			if (std::strcmp(linebuffer, "MESH") == 0)
			{
				...
				add.modelFile = add.modelFile.substr(0, add.modelFile.find_last_of(".")); //name found here
				tempName = add.modelFile;
				//names.push_back(add.modelFile);
				add.modelFile += ".h2b";
				...
				nameInfo.push_back(NameData(tempName, transform.row4));
